...
def get_payments_to_code_holders():
    ...
    payments = conn.execute(
        'SELECT * FROM payments_to_code_holders').fetchall()
    conn.close()
...
def get_internet_voice_bundles():
    conn = get_db_connection()
    payments = conn.execute(
        'SELECT * FROM bundle_purchases').fetchall()
    conn.close()
    results = [dict(payment) for payment in payments]
...
def get_cash_power_bill_payments():
    conn = get_db_connection()
    payments = conn.execute(
        'SELECT * FROM cashpower_payments').fetchall()
    conn.close()
    results = [dict(payment) for payment in payments]
...
def get_trantnxs_from_third_parties():
    conn = get_db_connection()
    payments = conn.execute(
        'SELECT * FROM third_party_transactions').fetchall()
    conn.close()
    results = [dict(payment) for payment in payments]
